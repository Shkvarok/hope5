                ...
                <div class="register-form">
                    ...
                    <form action="register.php" method="post" enctype="multipart/form-data" id="logform">
                        <div class="row">
                            ...
                            <div class="group-input col-md-6">
                                <label for="con">Номер телефону</label>
                        ...
                        <div class="group-input">
                            <label for="con-pass">Місце проживання та адреса *</label>
                            <input type="text" id="con-pass" name="address" required>
                        </div>
                        <div class="group-input">
                            <label for="con-pass">Зображення профілю</label>
                            <input type="file" name="pimage" style="border: none; margin-top:6px;">
                        </div>
                        <button type="submit" class="site-btn register-btn" name="register">Зареєструватися</button>
                    </form>
                    <div class="switch-login">
                        <a href="login.php" class="or-login">або авторизуватися</a>
                    </div>
                </div>
            </div>
        </div>
...
if (isset($_POST['register'])) {
    ...
    $defaultImage = "img/customer/image.png"; // Фото за замовчуванням
    $targetPath = $tardir . $fileName;
    $fileType = pathinfo($targetPath, PATHINFO_EXTENSION);
    $allow = array('jpg', 'png', 'jpeg');

    // Перевірка існування каталогу і створення його за потреби
    if (!is_dir($tardir)) {
        mkdir($tardir, 0777, true);
    }

    if (!empty($fileName) && in_array($fileType, $allow)) {
        if (move_uploaded_file($_FILES['pimage']['tmp_name'], $targetPath)) {
            $imagePath = $fileName;
        } else {
            echo "<script>alert('Файл не завантажено. Використовується фото за замовчуванням.')</script>";
    ...
    $check_email_query = "SELECT customer_email FROM customer WHERE customer_email='$c_email'";
    $run_check_email = mysqli_query($con, $check_email_query);
    if (mysqli_num_rows($run_check_email) > 0) {
        echo "<script>alert('Ця електронна адреса вже зареєстрована.')</script>";
        ...
        $insert_c = "INSERT INTO customer (customer_name, customer_email, customer_pass, customer_address, customer_contact, customer_image, customer_ip)
                     VALUES ('$c_name', '$c_email', '$c_pass', '$c_address', '$c_contact', '$imagePath', '$c_ip')";
        ...
        if ($run_insert) {
            ...
            if ($check_cart > 0) {
                $_SESSION['customer_email'] = $c_email;
                echo "<script>alert('Аккаунт зареєстровано. Ви увійшли.')</script>";
            ...
            echo "<script>alert('Помилка при реєстрації користувача.')</script>";
        }
    }
}
?>